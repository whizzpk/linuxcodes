...
{
    //reading   
    int readport = open_port("/dev/ttyS1");
...
{
    //open(rdr_devnode, O_RDWR|O_NOCTTY|O_NDELAY );
    int fd = open(str, O_RDWR | O_NOCTTY | O_NDELAY); // ?? NDELAY or NONBLOCK?
  ...
  cfsetispeed(&options, B1200); //input baudrate
      cfsetospeed(&options, B1200); // output baudrate
      ...
      options.c_cc[VMIN] = 11; // Wait until x bytes read (blocks!)
